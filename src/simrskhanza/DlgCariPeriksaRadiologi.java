...
import com.herinoid.rsi.gui.DlgFoundImageRadiologi;
import com.herinoid.rsi.gui.DlgGambarRadiologi;
    ...
    private void MnLihatGambarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_MnLihatGambarActionPerformed
        ...
        if (tabMode.getRowCount() == 0) {
            ...
            if (Kd2.getText().equals("") || Petugas.getText().equals("")) {
                ...
                String norawat = tbDokter.getValueAt(row, 0).toString();
                this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
                List<GambarRadiologi> gambarList = GambarRadiologiDao.getAllGambarRadiologi(norawat);
                if (!gambarList.isEmpty()) {
                    if (gambarList.size() == 1) {
                        GambarRadiologi gbr = gambarList.get(0);
                        DlgGambarRadiologi gambar = new DlgGambarRadiologi(null, true);
                        gambar.setGambar(gbr.getLokasiGambar());
                        ...
                        String pasien = tbDokter.getValueAt(row, 1).toString();
                        String petugas = tbDokter.getValueAt(row, 2).toString();
                        String tanggal = tbDokter.getValueAt(row, 3).toString();
                        DlgFoundImageRadiologi found = new DlgFoundImageRadiologi(null, true);
                        found.setData(norawat, tanggal, pasien, petugas, gambarList);
                        found.setSize(internalFrame1.getWidth() - 20, internalFrame1.getHeight() - 20);
                        found.setLocationRelativeTo(internalFrame1);
