...
public final class DlgCariObat3 extends javax.swing.JDialog {
    ...
    private double stokmasuk = 0, pagi = 0, siang = 0, sore = 0, malam = 0, keluar = 0, retur = 0, harga = 0, kapasitas = 0,
            enam = 0, delapan = 0, duabelas = 0, empatbelas = 0, delapanbelas = 0, duapuluh = 0, duapuluhdua = 0, duapuluhempat = 0,
    ...
    public DlgCariObat3(java.awt.Frame parent, boolean modal) {
        ...
        tabMode = new DefaultTableModel(null, row) {
            ...
            public boolean isCellEditable(int rowIndex, int colIndex) {
                boolean a = true;
                if ((colIndex == 1) || (colIndex == 2) || (colIndex == 3) || (colIndex == 12) || (colIndex == 13) || (colIndex == 14) || (colIndex == 15) || (colIndex == 16)) {
                    a = false;
                }
                return a;
            }
        ...
        if (TNoRw.getText().equals("")) {
            ...
            try {
                ...
                while (rspasien.next()) {
                    ...
                    param.put("tanggal", Valid.SetTgl(Tanggal.getSelectedItem() + ""));
            ...
            try {
                ...
                while (rstampilbarang.next()) {
                    ...
                    try {
                        ...
                        pspemberian.setString(5, "06:00:00");
                        rspemberian = pspemberian.executeQuery();
                        if (rspemberian.next()) {
                            enam = rspemberian.getDouble("jml");
                        }

                        pspemberian.setString(1, TNoRw.getText());
                        pspemberian.setString(2, Valid.SetTgl(Tanggal.getSelectedItem() + ""));
                        pspemberian.setString(3, rstampilbarang.getString("kode_brng"));
                        pspemberian.setString(4, "06:00:01");
                        pspemberian.setString(5, "08:00:00");
                        rspemberian = pspemberian.executeQuery();
                        if (rspemberian.next()) {
                            delapan = rspemberian.getDouble("jml");
                        }

                        pspemberian.setString(1, TNoRw.getText());
                        pspemberian.setString(2, Valid.SetTgl(Tanggal.getSelectedItem() + ""));
                        pspemberian.setString(3, rstampilbarang.getString("kode_brng"));
                        pspemberian.setString(4, "08:00:01");
                        pspemberian.setString(5, "12:00:00");
                        rspemberian = pspemberian.executeQuery();
                        if (rspemberian.next()) {
                            duabelas = rspemberian.getDouble("jml");
                        }

                        pspemberian.setString(1, TNoRw.getText());
                        pspemberian.setString(2, Valid.SetTgl(Tanggal.getSelectedItem() + ""));
                        pspemberian.setString(3, rstampilbarang.getString("kode_brng"));
                        pspemberian.setString(4, "12:00:01");
                        pspemberian.setString(5, "14:00:00");
                        rspemberian = pspemberian.executeQuery();
                        if (rspemberian.next()) {
                            empatbelas = rspemberian.getDouble("jml");
                        }

                        pspemberian.setString(1, TNoRw.getText());
                        pspemberian.setString(2, Valid.SetTgl(Tanggal.getSelectedItem() + ""));
                        pspemberian.setString(3, rstampilbarang.getString("kode_brng"));
                        pspemberian.setString(4, "14:00:01");
                        pspemberian.setString(5, "18:00:00");
                        rspemberian = pspemberian.executeQuery();
                        if (rspemberian.next()) {
                            delapanbelas = rspemberian.getDouble("jml");
                        }

                        pspemberian.setString(1, TNoRw.getText());
                        pspemberian.setString(2, Valid.SetTgl(Tanggal.getSelectedItem() + ""));
                        pspemberian.setString(3, rstampilbarang.getString("kode_brng"));
                        pspemberian.setString(4, "18:00:01");
                        pspemberian.setString(5, "20:00:00");
                        rspemberian = pspemberian.executeQuery();
                        if (rspemberian.next()) {
                            duapuluh = rspemberian.getDouble("jml");
                        }

                        pspemberian.setString(1, TNoRw.getText());
                        pspemberian.setString(2, Valid.SetTgl(Tanggal.getSelectedItem() + ""));
                        pspemberian.setString(3, rstampilbarang.getString("kode_brng"));
                        pspemberian.setString(4, "20:00:01");
                        pspemberian.setString(5, "22:00:00");
                        rspemberian = pspemberian.executeQuery();
                        if (rspemberian.next()) {
                            duapuluhdua = rspemberian.getDouble("jml");
                        }

                        pspemberian.setString(1, TNoRw.getText());
                        pspemberian.setString(2, Valid.SetTgl(Tanggal.getSelectedItem() + ""));
                        pspemberian.setString(3, rstampilbarang.getString("kode_brng"));
                        pspemberian.setString(4, "22:00:01");
                        pspemberian.setString(5, "23:59:59");
                        rspemberian = pspemberian.executeQuery();
                        if (rspemberian.next()) {
                            duapuluhempat = rspemberian.getDouble("jml");
                        }
                    } catch (Exception e) {
                        enam = 0;
                        delapan = 0;
                    ...
                    tabMode.addRow(new Object[]{false, rstampilbarang.getString("kode_brng"), rstampilbarang.getString("nama_brng"), stokmasuk,
                        enam, delapan, duabelas, empatbelas, delapanbelas, duapuluh, duapuluhdua, duapuluhempat, rstampilbarang.getDouble("jumlah"), keluar, retur, returshs, hilang, 0, 0});
                }
            } catch (Exception e) {
                System.out.println(e);
            } finally {
