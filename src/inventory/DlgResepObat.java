...
public final class DlgResepObat extends javax.swing.JDialog {
    ...
    public DlgResepObat(java.awt.Frame parent, boolean modal) {
        ...
        NoResep.setDocument(new batasInput((byte)12).getKata(NoResep));
    ...
    private void initComponents() {
        ...
        popUpEtiket = new javax.swing.JMenuItem();
        ...
        popUpEtiket.setBackground(new java.awt.Color(255, 255, 255));
        popUpEtiket.setFont(new java.awt.Font("Tahoma", 0, 11)); // NOI18N
        popUpEtiket.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/category.png"))); // NOI18N
        popUpEtiket.setText("Print Etiket");
        popUpEtiket.setName("popUpEtiket"); // NOI18N
        popUpEtiket.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                popUpEtiketActionPerformed(evt);
            }
        });
        Popup2.add(popUpEtiket);
        ...
        DTPBeri.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "24-06-2020" }));
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        if(tabMode.getRowCount()==0){
             ...
             JOptionPane.showMessageDialog(null,"Maaf, Gagal menghapus. Pilih dulu data yang mau dihapus.Klik data pada table untuk memilih...!!!!");
                    ...
                    int input = JOptionPane.showConfirmDialog(null, "Apakah anda yakin mau menghapus Resep Ini??", "Hapus Dialog", JOptionPane.OK_CANCEL_OPTION, JOptionPane.ERROR_MESSAGE);
                    if (input == 0) {
...
private void ChkInputActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ChkInputActionPerformed
  isForm();                
        ...
        if(tabMode.getRowCount()==0){
            ...
            try {
                ...
                try {
                    ...
                    while(rs.next()){
                        Sequel.menyimpan("temporary_resep","?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?",38,new String[]{
                            "0",rs.getString("nama_brng"),rs.getString("aturan"),rs.getString("jml"),rs.getString("satuan"),"","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","","",""
                        });
                        System.out.println("Data "+rs.getString("nama_brng"));
                    }
                } catch (Exception e) {
                ...
                ps=koneksi.prepareStatement(
                    "select obat_racikan.nama_racik," +
                    "obat_racikan.aturan_pakai,obat_racikan.jml_dr,metode_racik.nm_racik " +
                    "from resep_obat inner join reg_periksa inner join " +
                    "obat_racikan inner join metode_racik on resep_obat.no_rawat=reg_periksa.no_rawat " +
                    "and obat_racikan.kd_racik=metode_racik.kd_racik " +
            ...
            Valid.MyReport("rptLembarObat2.jasper",param,"::[ Lembar Pemberian Obat ]::");
            this.setCursor(Cursor.getDefaultCursor());
        }
    ...
    private void popUpEtiketActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_popUpEtiketActionPerformed
        // TODO add your handling code here:
        System.out.println("masuk klik etiket print");
        if (tabMode.getRowCount() == 0) {
            JOptionPane.showMessageDialog(null, "Maaf, data sudah habis...!!!!");
            TNoRw.requestFocus();
        } else if (TPasien.getText().trim().equals("")) {
            ...
            param.put("emailrs", akses.getemailrs());
            param.put("logo", Sequel.cariGambar("select logo from setting"));
            param.put("tanggal", Utils.formatTanggal(Sequel.cariTanggal("SELECT tgl_peresepan FROM resep_obat WHERE no_resep=?", NoResep.getText())));
            if (Sequel.cariInteger(
                    "select count(*) from resep_obat inner join "
                    + "aturan_pakai on resep_obat.no_rawat=aturan_pakai.no_rawat and "
                    + "resep_obat.tgl_perawatan=aturan_pakai.tgl_perawatan and "
                    + "resep_obat.jam=aturan_pakai.jam where resep_obat.no_resep=? and aturan_pakai.aturan<>''", NoResep.getText()) > 0) {
                Valid.MyReportqry("etiketUmum.jasper", "report", "::[ Etiket Obat ]::",
                        "SELECT resep_obat.no_resep,resep_obat.tgl_perawatan,resep_obat.jam, "
                        + "resep_obat.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,databarang.nama_brng,"
                        + "aturan_pakai.aturan,detail_pemberian_obat.jml,kodesatuan.satuan,databarang.`expire`,"
                        + "IF(reg_periksa.`status_lanjut`='ranap',(SELECT bangsal.`nm_bangsal` FROM kamar_inap JOIN kamar ON kamar_inap.`kd_kamar`=kamar.`kd_kamar` JOIN bangsal ON bangsal.`kd_bangsal`=kamar.`kd_bangsal` "
                        + "WHERE kamar_inap.`no_rawat`=reg_periksa.`no_rawat` ORDER BY kamar_inap.`jam_masuk` DESC LIMIT 1) ,(SELECT p.`nm_poli` FROM poliklinik p JOIN reg_periksa r ON p.`kd_poli`=r.`kd_poli` WHERE r.no_rawat=reg_periksa.`no_rawat`)) AS lokasi "
                        + "FROM resep_obat INNER JOIN reg_periksa INNER JOIN pasien INNER JOIN "
                        + "aturan_pakai INNER JOIN databarang INNER JOIN detail_pemberian_obat "
                        + "INNER JOIN kodesatuan ON resep_obat.no_rawat=reg_periksa.no_rawat  "
                        + "AND reg_periksa.no_rkm_medis=pasien.no_rkm_medis AND "
            ...
            if (Sequel.cariInteger(
                    ...
                    + "resep_obat.tgl_perawatan=obat_racikan.tgl_perawatan and "
                    + "resep_obat.jam=obat_racikan.jam where resep_obat.no_resep=? and obat_racikan.aturan_pakai<>''", NoResep.getText()) > 0) {
                Valid.MyReportqry("etiketRacikan.jasper", "report", "::[ Aturan Pakai Obat ]::",
                        "SELECT resep_obat.no_resep,resep_obat.tgl_perawatan,resep_obat.jam,"
                        + "resep_obat.no_rawat,pasien.no_rkm_medis,pasien.nm_pasien,obat_racikan.nama_racik,"
                        + "obat_racikan.aturan_pakai,obat_racikan.jml_dr,metode_racik.nm_racik,databarang.nama_brng,databarang.`expire`,"
                        + "IF(reg_periksa.`status_lanjut`='ranap',(SELECT bangsal.`nm_bangsal` FROM kamar_inap JOIN kamar ON kamar_inap.`kd_kamar`=kamar.`kd_kamar` JOIN bangsal ON bangsal.`kd_bangsal`=kamar.`kd_bangsal` "
                        + "WHERE kamar_inap.`no_rawat`=reg_periksa.`no_rawat` ORDER BY kamar_inap.`jam_masuk` DESC LIMIT 1) ,(SELECT p.`nm_poli` FROM poliklinik p WHERE p.kd_poli=reg_periksa.`kd_poli` LIMIT 1)) AS lokasi "
                        + "FROM resep_obat INNER JOIN reg_periksa INNER JOIN pasien INNER JOIN databarang "
                        + "INNER JOIN obat_racikan INNER JOIN metode_racik INNER JOIN detail_obat_racikan ON resep_obat.no_rawat=reg_periksa.no_rawat "
                        + "AND reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                        + "AND obat_racikan.kd_racik=metode_racik.kd_racik "
                        + "AND resep_obat.no_rawat=obat_racikan.no_rawat AND "
                        + "resep_obat.tgl_perawatan=obat_racikan.tgl_perawatan AND "
                        + "resep_obat.jam=obat_racikan.jam AND resep_obat.no_rawat=obat_racikan.no_rawat "
                        + "AND detail_obat_racikan.`no_rawat`=obat_racikan.`no_rawat` "
                        + "AND databarang.`kode_brng`=detail_obat_racikan.`kode_brng` "
                        + "WHERE resep_obat.no_resep='" + NoResep.getText() + "'", param);
            }
            this.setCursor(Cursor.getDefaultCursor());
        }
    }//GEN-LAST:event_popUpEtiketActionPerformed
    ...
    */
    ...
    private javax.swing.JMenuItem popUpEtiket;
