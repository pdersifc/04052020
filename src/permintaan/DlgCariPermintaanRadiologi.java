        ...
        if (TabPilihRawat.getSelectedIndex() == 0) {
            if (tbRadiologiRalan.getSelectedRow() != -1) {
                if (tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 0).toString().trim().equals("")) {
                    ...
                    param.put("kontakrs", akses.getkontakrs());
                    param.put("emailrs", akses.getemailrs());
                    Valid.MyReportqry("rptBarcodePermintaanRadiologirj.jasper", "report", "::[ Barcode No.Permintaan Radiologi ]::",
                            "select permintaan_radiologi.noorder,permintaan_radiologi.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,\n"
                            + "penjab.png_jawab,pasien.tgl_lahir,pasien.umur,pasien.alamat,poliklinik.nm_poli,\n"
                            + "jns_perawatan_radiologi.nm_perawatan,permintaan_radiologi.dokter_perujuk,dokter.nm_dokter,\n"
                            + "permintaan_radiologi.tgl_permintaan,permintaan_radiologi.status\n"
                            + "from permintaan_radiologi inner join reg_periksa inner join permintaan_pemeriksaan_radiologi inner join poliklinik\n"
                            + "inner join pasien inner join jns_perawatan_radiologi inner join penjab inner join dokter on permintaan_radiologi.no_rawat=reg_periksa.no_rawat\n"
                            + "and reg_periksa.no_rkm_medis=pasien.no_rkm_medis and permintaan_pemeriksaan_radiologi.kd_jenis_prw=jns_perawatan_radiologi.kd_jenis_prw\n"
                            + "and permintaan_radiologi.dokter_perujuk=dokter.kd_dokter \n"
                            + "and permintaan_radiologi.noorder=permintaan_pemeriksaan_radiologi.noorder\n"
                            + "and reg_periksa.kd_pj=penjab.kd_pj and reg_periksa.kd_poli=poliklinik.kd_poli\n"
                            + "where permintaan_radiologi.no_rawat='" + tbRadiologiRalan.getValueAt(tbRadiologiRalan.getSelectedRow(), 1).toString() + "'", param);
                    this.setCursor(Cursor.getDefaultCursor());
                }
            } else {
                JOptionPane.showMessageDialog(null, "Maaf, silahkan pilih data permintaan...!!!!");
            ...
            if (tbRadiologiRanap.getSelectedRow() != -1) {
                if (tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 0).toString().trim().equals("")) {
                    ...
                    param.put("kontakrs", akses.getkontakrs());
                    param.put("emailrs", akses.getemailrs());
                    Valid.MyReportqry("rptBarcodePermintaanRadiologiri.jasper", "report", "::[ Barcode No.Permintaan Radiologi ]::",
                            "select permintaan_radiologi.noorder,permintaan_radiologi.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,\n"
                            + "penjab.png_jawab,(select bangsal.nm_bangsal from kamar_inap inner join kamar inner join bangsal on kamar_inap.kd_kamar=kamar.kd_kamar and kamar.kd_bangsal=bangsal.kd_bangsal \n"
                            + "where kamar_inap.no_rawat=permintaan_radiologi.no_rawat limit 1 ) as nm_bangsal,\n"
                            + "pasien.tgl_lahir,pasien.umur,pasien.alamat,poliklinik.nm_poli,\n"
                            + "jns_perawatan_radiologi.nm_perawatan,permintaan_radiologi.dokter_perujuk,dokter.nm_dokter,\n"
                            + "permintaan_radiologi.tgl_permintaan,permintaan_radiologi.status\n"
                            + "from permintaan_radiologi inner join reg_periksa inner join permintaan_pemeriksaan_radiologi inner join poliklinik\n"
                            + "inner join pasien inner join jns_perawatan_radiologi inner join penjab inner join dokter on permintaan_radiologi.no_rawat=reg_periksa.no_rawat\n"
                            ...
                            + "and permintaan_radiologi.noorder=permintaan_pemeriksaan_radiologi.noorder\n"
                            + "and reg_periksa.kd_pj=penjab.kd_pj and reg_periksa.kd_poli=poliklinik.kd_poli\n"
                            + "where permintaan_radiologi.no_rawat='" + tbRadiologiRanap.getValueAt(tbRadiologiRanap.getSelectedRow(), 1).toString() + "'", param);
                    this.setCursor(Cursor.getDefaultCursor());
                }
            } else {
                JOptionPane.showMessageDialog(null, "Maaf, silahkan pilih data permintaan...!!!!");
        ...
        try {
            if (CrDokter.getText().trim().equals("") && CrPoli.getText().trim().equals("") && TCari.getText().trim().equals("")) {
                ps = koneksi.prepareStatement(
                        ...
                        + "where permintaan_radiologi.status='ralan' and permintaan_radiologi.tgl_permintaan between ? and ? order by permintaan_radiologi.tgl_permintaan,permintaan_radiologi.jam_permintaan desc");
            } else {
                ps = koneksi.prepareStatement(
                        "select permintaan_radiologi.noorder,permintaan_radiologi.no_rawat,reg_periksa.no_rkm_medis,pasien.nm_pasien,permintaan_radiologi.tgl_permintaan,"
                        + "if(permintaan_radiologi.jam_permintaan='00:00:00','',permintaan_radiologi.jam_permintaan) as jam_permintaan,"
                        ...
                        + "inner join dokter on permintaan_radiologi.dokter_perujuk=dokter.kd_dokter "
                        + "inner join poliklinik on reg_periksa.kd_poli=poliklinik.kd_poli "
                        + "where permintaan_radiologi.status='ralan' and permintaan_radiologi.tgl_permintaan between ? and ? and dokter.nm_dokter like ? and poliklinik.nm_poli like ? and permintaan_radiologi.noorder like ? or "
        ...
        try {
            if (CrDokter2.getText().trim().equals("") && Kamar.getText().trim().equals("") && TCari.getText().trim().equals("")) {
                ps = koneksi.prepareStatement(
                        ...
                        + "if(permintaan_radiologi.tgl_sampel='0000-00-00','',permintaan_radiologi.tgl_sampel) as tgl_sampel,if(permintaan_radiologi.jam_sampel='00:00:00','',permintaan_radiologi.jam_sampel) as jam_sampel,"
                        + "permintaan_radiologi.tgl_hasil,if(permintaan_radiologi.jam_hasil='00:00:00','',permintaan_radiologi.jam_hasil) as jam_hasil,"
                        + "permintaan_radiologi.dokter_perujuk,dokter.nm_dokter,bangsal.nm_bangsal,permintaan_radiologi.informasi_tambahan,permintaan_radiologi.diagnosa_klinis "
                        + "from permintaan_radiologi inner join reg_periksa on permintaan_radiologi.no_rawat=reg_periksa.no_rawat "
                        + "inner join pasien on reg_periksa.no_rkm_medis=pasien.no_rkm_medis "
                        + "inner join dokter on permintaan_radiologi.dokter_perujuk=dokter.kd_dokter "
